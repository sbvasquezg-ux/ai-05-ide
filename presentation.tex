...
\begin{frame}[fragile]{Lean: frontera verificable y bloqueo real}
\small
\textbf{1. Objeto original que se quería traducir}
\[
n(z)[a-w(z)]-r=0\ \Longrightarrow\ w(z)=a-h(1-z)r.
\]
\textbf{2. Enunciado Lean y prueba: no generados.} El agente se interrumpió por límite de uso antes de crear la carpeta. No se inventa un fragmento.
\begin{lstlisting}
-- No hay teorema ni prueba Lean generados.
-- Formalizacion pendiente; esto no es codigo verificado.
\end{lstlisting}
\textbf{3. Interpretación.} Había que representar reales, dominio $0<h<1$, $z<1$ y beneficio cero. Incluso probar esta identidad no demostraría el equilibrio ni la Prop. 5.

\medskip
\textbf{4. Check real: exit 2, Python 3.12.14.}
\begin{lstlisting}[language={}]
error: could not read papers/IT25KnowledgeEconomy/status.json
[Errno 2] No such file or directory
\end{lstlisting}
\sourcefoot{Ecuación: §3.1, p. 12; Prop. 2, p. 20. Registro: lean/CHECK\_OUTPUT.txt.}
\end{frame}

\begin{frame}{La evidencia manual y lo que no acepté de la IA}
\begin{columns}[T,onlytextwidth]
\column{.53\textwidth}
\small
\textbf{Objeción evaluada:} la frase ``autonomía, no capacidad'' omite dos umbrales.
\[
 (1-h)H<H.
\]
\textbf{Mi veredicto analítico:} ambos regímenes requieren capacidad para una ganancia estricta en la base; sus mecanismos y umbrales difieren.

\medskip
\textbf{Corrección durante la auditoría:} mi cálculo uniforme omitió $ht^2/2$. El contraste con el LP detectó el fallo; no era un error del paper.
\column{.43\textwidth}
\IfFileExists{hand/manual-verification.png}{\includegraphics[width=\linewidth,height=.58\textheight,keepaspectratio]{hand/manual-verification.png}}{%
\begin{block}{Foto pendiente de la estudiante}
\small Copiar la derivación de dos tipos de \texttt{hand/README.md}, terminar en los dos umbrales y añadir su fotografía real.

\medskip
El repositorio no inventa evidencia manuscrita.
\end{block}}
\end{columns}
\sourcefoot{Derivación propia; contraste interpretativo con Props. 5--6, pp. 24 y 27.}
\end{frame}
...
\item Pendientes para la estudiante: foto real, formalización Lean y revisión de las fronteras de régimen.
\end{itemize}

...
The six source-facing rows below are scaffold drafts. They are not accepted
source translations and none is proved. The compiled, `sorry`-free content is
limited to the local algebraic lemmas listed after the table.

| Paper item | Lean declaration | Status | File | Remaining assumptions / notes |
|---|---|---|---|---|
| Proposition 1 (page 16, Section 3.3, Proposition 1) | `proposition1_preAI_equilibriumSpec` -> `proposition1_preAI_equilibrium` | scaffold | `PaperInterface.lean` | Semantically incomplete/incorrect draft over arbitrary carrier and predicates; endpoint is `by sorry`; no source approval or proof credit |
| Proposition 2 (pages 18-19, Section 4, Proposition 2) | `proposition2_autonomousAI_equilibriumSpec` -> `proposition2_autonomousAI_equilibrium` | scaffold | `PaperInterface.lean` | Semantically incomplete/incorrect draft without the paper's defined equilibrium model; endpoint is `by sorry`; no source approval or proof credit |
| Proposition 3 (page 22, Section 5.1, Proposition 3) | `proposition3_occupational_displacementSpec` -> `proposition3_occupational_displacement` | scaffold | `PaperInterface.lean` | Semantically incomplete/incorrect draft over arbitrary sets/functions; endpoint is `by sorry`; no source approval or proof credit |
| Proposition 4 (page 22, Section 5.1, Proposition 4) | `proposition4_productivity_and_spanSpec` -> `proposition4_productivity_and_span` | scaffold | `PaperInterface.lean` | Semantically incomplete/incorrect draft over arbitrary functions; endpoint is `by sorry`; no source approval or proof credit |
| Proposition 5 (page 24, Section 5.2, Proposition 5) | `proposition5_winners_at_extremesSpec` -> `proposition5_winners_at_extremes` | scaffold | `PaperInterface.lean` | Does not encode the paper's connected extreme winner intervals and threshold structure; endpoint is `by sorry`; no proof credit |
| Proposition 6 (page 27, Section 6.1, Proposition 6) | `proposition6_nonautonomousAI_equilibriumSpec` -> `proposition6_nonautonomousAI_equilibrium` | scaffold | `PaperInterface.lean` | Semantically incomplete/incorrect draft without equilibrium definitions; endpoint is `by sorry`; no source approval or proof credit |

### Compiled algebraic scope

`MainTheorems.lean` proves the zero-profit wage bridge, its non-autonomous and
autonomous specializations, two conditional bottom-wage equivalences, the
strict comparison between autonomous and non-autonomous bottom thresholds, and
three conditional two-type candidate-offer comparisons. These results are
standalone algebraic consequences under explicit premises. They do not prove
equilibrium existence, uniqueness, efficiency, assignment, welfare, or any
complete proposition from the paper.
